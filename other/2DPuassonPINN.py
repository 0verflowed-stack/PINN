...
def dirichlet_condition(model, x1_x2_bc_up_down):
    u, _, _, _, _ = model.forward(x1_x2_bc_up_down)
    return tf.reduce_mean(tf.square(u))

def neuman_condition(model, x1_x2_bc_left_right):
    _, u_x1, _,  _, _ = model.forward(x1_x2_bc_left_right)
    return tf.reduce_mean(tf.square(u_x1)) # -∂u/∂x1 + ∂u/∂x1


def loss_fn(u_x1_x1, u_x2_x2, model, f, x1_x2_bc_left_right, x1_x2_bc_up_down):
    puasson_eq = u_x1_x1 + u_x2_x2 - f(x1_x2_train)
    bc_dirichlet = dirichlet_condition(model, x1_x2_bc_up_down)
    bc_neuman = neuman_condition(model, x1_x2_bc_left_right)

    return 10 * tf.reduce_mean(tf.square(puasson_eq)) + 2 * (bc_dirichlet + bc_neuman)
...
x1_train = np.linspace(0, L_x1, 25)[:, np.newaxis]
x2_train = np.linspace(0, L_x2, 25)[:, np.newaxis]
...
while loss.numpy() > 0.005:
    with tf.GradientTape() as tape:
        u, u_x1, u_x2, u_x1_x1, u_x2_x2 = model.forward(x1_x2_train)
        loss = loss_fn(u_x1_x1, u_x2_x2, model, f, x1_x2_bc_left_right, x1_x2_bc_up_down)
...
U_exact = exact_solution(x1_mesh, x2_mesh)
