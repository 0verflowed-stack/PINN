...
class Puasson1DPINN(tf.keras.Model):
    ...
    def f(self, x):
        return -np.pi * np.sin(np.pi * x) * np.sin(np.pi * 0.5)

    def exact_solution(self, x):
        return np.sin(np.pi * x)/np.pi

N_of_train_points_1D = 50
N_of_test_points_1D = 100
L_x_1D = 0.0
R_x_1D = 1.0
...
class Puasson2DPINN(tf.keras.Model):
    ...
    def dirichlet_condition(self, x1_x2_bc_up_down):
    ...
    def loss_fn(self, x1_x2_train, u_x1_x1, u_x2_x2, x1_x2_bc_left_right, x1_x2_bc_up_down):
        puasson_eq = u_x1_x1 + u_x2_x2 - self.f(x1_x2_train)
        bc_dirichlet = self.dirichlet_condition(x1_x2_bc_up_down)
        bc_neuman = self.neuman_condition(x1_x2_bc_left_right)

        return 1 * tf.reduce_mean(tf.square(puasson_eq)) + 2 * (bc_dirichlet + bc_neuman)

    def train(self, x1_x2_train, x1_x2_bc_left_right, x1_x2_bc_up_down):
        loss_array = []
        start_time = time.time()
        ...
        while loss.numpy() > 0.001:
            with tf.GradientTape() as tape:
                ...
                loss = self.loss_fn(x1_x2_train, u_x1_x1, u_x2_x2, x1_x2_bc_left_right, x1_x2_bc_up_down)
            grads = tape.gradient(loss, self.trainable_variables)
            self.optimizer.apply_gradients(zip(grads, self.trainable_variables))
...
model_2D.train(x1_x2_train, x1_x2_bc_left_right, x1_x2_bc_up_down)
...
x1_test = np.linspace(L_x1_2D, R_x1_2D, 100)[:, np.newaxis]
x2_test = np.linspace(L_x2_2D, R_x2_2D, 100)[:, np.newaxis]
